...
@router.get("/", response_model=List[user_schemas.UserReadWithRoles], dependencies=[Depends(RoleChecker(["admin"]))])
def get_all_users(db: DbSession):
    users = db.exec(select(User)).all()
    return users

# Endpoint này dành cho ADMIN hoặc STAFF
@router.get("/staff-info", response_model=dict, dependencies=[Depends(RoleChecker(["admin", "staff"]))])
def get_staff_info():
