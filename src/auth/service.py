...
def register_user(
    session: Session, register_request: schemas.RegisterUserRequest
) -> User:
    try:
        ...
        if not customer_role:
            raise HTTPException(
                status_code=500, detail="Default role 'customer' not found."
            )
        ...
        session.rollback()
        logging.error(
            f"Failed to register user: {register_request.email}. Error: {str(e)}"
        )
        raise
